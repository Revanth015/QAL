...
function Profile() {
  ...
  return <Layout><div className="mx-auto max-w-5xl space-y-6">
    ...
    <section className="rounded-xl bg-white p-6 shadow"><div className="flex items-center justify-between"><div><p className="text-sm font-semibold text-purple-600">Collector Cabinet</p><h2 className="mt-1 text-xl font-bold">Your Badges</h2></div><span className="rounded-full bg-purple-50 px-3 py-1 text-xs font-bold text-purple-700">{badges.length} collected</span></div><div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{badges.map((badge) => <div key={badge.id} className="rounded-xl border border-gray-100 p-4 text-center"><div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-purple-50 text-3xl">{badge.icon}</div><p className="mt-3 font-bold">{badge.name}</p><p className="mt-1 text-xs text-gray-400">{badge.rarity}</p></div>)}</div></section>
